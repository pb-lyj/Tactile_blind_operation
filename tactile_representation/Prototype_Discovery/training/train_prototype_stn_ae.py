"""
Improved STN Prototype Autoencoder 训练脚本
"""
...
def train_prototype_stn_ae(config):
    """
    训练改进版STN原型自编码器
    Args:
        config: 配置字典
    ...
    model = ImprovedPrototypeSTNAE(
        ...
        share_stn=config['model']['share_stn']
    ).cuda()
    
    print(f"模型参数数量: {sum(p.numel() for p in model.parameters()):,}")
    print(f"共享STN: {config['model']['share_stn']}")
    ...
    # 记录损失历史
    loss_fields = ['recon_loss', 'diversity_loss', 'entropy_loss', 'sparsity_loss', 'gini_coeff', 'stn_loss']
    ...
    for epoch in range(1, config['training']['epochs'] + 1):
        ...
        for batch in tqdm(loader, desc=f"Epoch {epoch}/{config['training']['epochs']}"):
            ...
            # 前向传播
            recon, weights, transformed_protos, thetas = model(inputs)
            
            # 计算损失
            loss, metrics = compute_improved_stn_losses(
                inputs, recon, weights, transformed_protos, thetas,
                ...
                stn_reg_lambda=config['loss']['stn_reg_lambda']
            )
...
def main(config):
    ...
    # 设置日志
    log_file = os.path.join(config['output']['output_dir'], "train_improved_stn.log")
    sys.stdout = Logger(log_file)
    
    print("=" * 60)
    print("Improved STN Prototype Autoencoder Training")
    print(f"Output Directory: {config['output']['output_dir']}")
    print(f"Prototypes: {config['model']['num_prototypes']}")
    print(f"Feature Dim: {config['model']['feature_dim']}")
    print(f"Share STN: {config['model']['share_stn']}")
    ...
    # 训练模型
    model, loss_history = train_prototype_stn_ae(config)
    ...
    plot_all_losses_single_plot(
        ...
        title="Improved STN Prototype AE Training Loss"
    )
    ...
    config = {
        ...
        'output': {
            'output_dir': os.path.join("./tactile_representation/prototype_library", 
                                     datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "_improved_stn")
        }
    }
