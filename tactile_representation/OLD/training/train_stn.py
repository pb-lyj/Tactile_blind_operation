"""
STN Prototype Autoencoder 训练脚本
"""
...
def main(config):
    ...
    # 设置日志
    log_file = os.path.join(config['output']['output_dir'], "train_stn.log")
    sys.stdout = Logger(log_file)
    
    print("=" * 60)
    print("STN Prototype Autoencoder Training")
    ...
    # 训练模型
    model, loss_history = train_stn(config)
    ...
    plot_all_losses_single_plot(
        ...
        title="STN Prototype AE Training Loss"
    )
    ...
    config = {
        ...
        'output': {
            'output_dir': os.path.join("./tactile_representation/prototype_library", 
                                     datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "_stn")
        }
    }
