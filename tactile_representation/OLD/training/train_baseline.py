"""
Baseline Prototype Autoencoder 训练脚本
CNN Encoder + 可学习原型库
"""
...
def train_baseline(config):
    ...
    # 记录损失历史
    loss_fields = ['recon_loss', 'diversity_loss', 'entropy_loss']
...
def main(config):
    ...
    # 设置日志
    log_file = os.path.join(config['output']['output_dir'], "train_baseline.log")
    sys.stdout = Logger(log_file)
    
    print("=" * 60)
    print("Baseline Prototype Autoencoder Training")
    ...
    plot_all_losses_single_plot(
        ...
        title="Baseline Prototype AE Training Loss"
    )
    ...
    config = {
        ...
        'output': {
            'output_dir': os.path.join("./tactile_representation/prototype_library", 
                                     datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "_baseline")
        }
    }
