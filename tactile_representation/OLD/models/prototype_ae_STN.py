...
class SharedSpatialTransformer(nn.Module):
    def __init__(self, input_size=(20, 20)):
        ...
        # 计算经过卷积后的特征大小
        # 对于20x20输入: 20 -> 10 (pool) -> 5 (pool)
        # 对于32x32输入: 32 -> 16 (pool) -> 8 (pool)
        pool_h = input_size[0] // 4
        pool_w = input_size[1] // 4
        feature_size = 32 * pool_h * pool_w
...
class PrototypeAutoencoder(nn.Module):
    def __init__(self, num_prototypes=16, input_shape=(3, 32, 32), share_stn=True):
        ...
        # STN 模块
        if share_stn:
            self.shared_stn = SharedSpatialTransformer(input_size=(self.H, self.W))
            # 为每个原型添加定位头
            for _ in range(num_prototypes):
                self.shared_stn.add_localization_head()
        else:
            self.stn_modules = nn.ModuleList([SpatialTransformer(input_size=(self.H, self.W)) for _ in range(num_prototypes)])
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Flatten(),
            self.final_linear,
            nn.Softmax(dim=-1)  # 改用Softmax，避免Sigmoid饱和
            # nn.Sigmoid()
        )
        
        # 调整初始化，适应Softmax
        nn.init.xavier_uniform_(self.final_linear.weight)
        self.final_linear.bias.data.fill_(0.0)  # Softmax用0初始化

    def forward(self, x):
        weights = self.encoder(x)  # (B, K) - Softmax输出，已归一化
        B = x.size(0)
...
def compute_losses(x, recon, weights, transformed_protos, thetas,
    ...
    # Reconstruction loss
    recon_loss = F.smooth_l1_loss(recon, x)  # Huber损失更鲁棒
    ...
    # Entropy loss on weights - 适应Softmax输出
    # 使用标准熵计算，鼓励权重分布不要过于集中在单一原型上
    entropy_loss = entropy_lambda * (-(weights * torch.log(weights + 1e-8)).sum(dim=1).mean())
