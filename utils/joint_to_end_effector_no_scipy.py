#!/usr/bin/env python3
"""
KUKA iiwa14 Forward Kinematics (无scipy依赖版本)
...
def euler_from_matrix(matrix):
    """
    从旋转矩阵提取欧拉角 (XYZ顺序)
    替代scipy.spatial.transform.Rotation的功能
    """
    # 提取欧拉角 (XYZ顺序)
...
class KUKAiiwa14ForwardKinematics:
    ...
    def forward_kinematics(self, joint_angles):
        ...
        # 使用自定义函数转换为欧拉角（XYZ顺序）
        orientation = euler_from_matrix(rotation_matrix)
...
def process_single_directory(directory_path):
    ...
    # 处理指定目录
    process_directory(directory_path, fk_calculator)
...
if __name__ == "__main__":
    # 如果是为了处理缺失的目录，可以直接调用
    missing_dir = "/home/lyj/Program_python/Tactile_blind_operation/datasets/data25.7_aligned/cir_med/20250722_121413_277_14186_2f9f8fc5"
    if os.path.exists(missing_dir):
        print("Processing missing directory...")
        process_single_directory(missing_dir)
    else:
        # 否则运行完整处理
        main()
